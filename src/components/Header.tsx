      ...
      <div className="section-container">
        <nav className="flex items-center justify-between h-16 md:h-20">
          ...
          <a href="#" className="flex items-center gap-2 group">
            ...
            <span className="font-display text-xl font-bold text-foreground">Taller El Chele</span>
          </a>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} className="font-medium transition-colors text-foreground">
                {item.label}
              </a>
            ))}
